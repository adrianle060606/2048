...
class Board(object):
    ...
    def add_animation(self, init_pos, final_pos, init_tile_value, final_tile_value, direction):
        self.animations.append({"current_pos": init_pos , "final_pos": final_pos, "init_tile_value": init_tile_value, "final_tile_value": final_tile_value, "direction": direction})

    def animate_tiles(self):
        for animation in self.animations:
            print(animation.current_pos)
            position_shift = (animation.direction[0] * constants.ANIMATION_SPEED, animation.direction[1] * constants.ANIMATION_SPEED)
            new_position = tuple(map(lambda i, j: i + j, animation.current_pos, position_shift))
            
            self.draw_tile(new_position[0], new_position[1], str(animation.init_tile_value))
